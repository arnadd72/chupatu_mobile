...
// IMPORT WAJIB UNTUK LOGOUT GOOGLE
import 'package:google_sign_in/google_sign_in.dart';

// --- IMPORT MAIN & HALAMAN LAIN ---
import 'package:chupatu_mobile/main.dart'; 
import 'package:chupatu_mobile/pages/auth/landing_page.dart'; // Arahkan ke Landing setelah logout
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  final Map<String, List<String>> _indonesiaData = {
    ...
    'Jawa Barat': ['Bandung', 'Bandung Barat', 'Kab. Bandung', 'Cimahi', 'Bogor', 'Kab. Bogor', 'Depok', 'Bekasi', 'Kab. Bekasi', 'Cikarang', 'Sukabumi', 'Cianjur', 'Tasikmalaya', 'Ciamis', 'Garut', 'Cirebon', 'Indramayu', 'Majalengka', 'Kuningan', 'Karawang', 'Purwakarta', 'Subang', 'Sumedang'],
    'Banten': ['Tangerang', 'Tangerang Selatan', 'Kab. Tangerang', 'Serang', 'Cilegon', 'Pandeglang', 'Lebak'],
    'Jawa Tengah': ['Semarang', 'Kab. Semarang', 'Salatiga', 'Kendal', 'Demak', 'Grobogan', 'Solo (Surakarta)', 'Sukoharjo', 'Boyolali', 'Klaten', 'Wonogiri', 'Sragen', 'Karanganyar', 'Magelang', 'Temanggung', 'Wonosobo', 'Purworejo', 'Kebumen', 'Banyumas (Purwokerto)', 'Cilacap', 'Purbalingga', 'Banjarnegara', 'Tegal', 'Brebes', 'Pekalongan', 'Pemalang', 'Batang', 'Kudus', 'Jepara', 'Pati', 'Rembang', 'Blora'],
    'DI Yogyakarta': ['Yogyakarta', 'Sleman', 'Bantul', 'Kulon Progo', 'Gunung Kidul'],
    'Jawa Timur': ['Surabaya', 'Sidoarjo', 'Gresik', 'Malang', 'Batu', 'Kab. Malang', 'Pasuruan', 'Probolinggo', 'Kediri', 'Blitar', 'Tulungagung', 'Trenggalek', 'Nganjuk', 'Madiun', 'Ponorogo', 'Ngawi', 'Magetan', 'Pacitan', 'Mojokerto', 'Jombang', 'Bojonegoro', 'Tuban', 'Lamongan', 'Banyuwangi', 'Jember', 'Bondowoso', 'Situbondo', 'Lumajang', 'Madura (Bangkalan)', 'Madura (Sampang)', 'Madura (Pamekasan)', 'Madura (Sumenep)'],
  ...
  // --- 5. LOGOUT (DENGAN CLEAR GOOGLE SESSION) ---
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                // 2. Clear Session Google (PENTING!)
                final GoogleSignIn googleSignIn = GoogleSignIn();
                await googleSignIn.signOut();
                // await googleSignIn.disconnect(); // Opsional: Hapus izin akses total
  ...
  // ... (SISA KODE UI DI BAWAH INI TETAP SAMA SEPERTI SEBELUMNYA) ...
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        return StatefulBuilder(builder: (BuildContext context, StateSetter setModalState) {
            return Container(height: MediaQuery.of(context).size.height * 0.85, padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: theme.surface, borderRadius: const BorderRadius.vertical(top: Radius.circular(20))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text("Ubah Alamat", style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.bold, color: theme.textMain)), IconButton(icon: Icon(Icons.close, color: theme.textMain), onPressed: () => Navigator.pop(context))]),
                const SizedBox(height: 20),
                Expanded(child: ListView(children: [
                    _buildLabel("Provinsi"),
                    _buildDropdown(theme, selectedProv, "Pilih Provinsi", _indonesiaData.keys.toList(), (val) { setModalState(() { selectedProv = val; selectedCity = null; selectedDist = null; }); }),
                    const SizedBox(height: 16),
                    _buildLabel("Kota / Kabupaten"),
                    _buildDropdown(theme, selectedCity, "Pilih Kota", selectedProv != null ? _indonesiaData[selectedProv]! : [], (val) { setModalState(() { selectedCity = val; selectedDist = null; }); }),
                    const SizedBox(height: 16),
                    _buildLabel("Kecamatan"),
                    _buildDropdown(theme, selectedDist, "Pilih Kecamatan", selectedCity != null ? (_kecamatanData[selectedCity] ?? ['Lainnya']) : [], (val) { setModalState(() { selectedDist = val; }); }),
                    const SizedBox(height: 16),
                    _buildLabel("Detail Jalan / Gedung / No. Rumah"),
                    TextField(controller: streetController, maxLines: 3, style: GoogleFonts.plusJakartaSans(color: theme.textMain), decoration: InputDecoration(hintText: "Jl. Mawar No. 12, RT 01/02", filled: true, fillColor: theme.background, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none))),
                  ])),
                SizedBox(width: double.infinity, child: ElevatedButton(onPressed: () { if (selectedProv != null && selectedCity != null && streetController.text.isNotEmpty) { String fullAddress = "${streetController.text}, Kec. ${selectedDist ?? '-'}, $selectedCity, $selectedProv"; _saveFirestoreData('address', fullAddress); Navigator.pop(context); } else { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Lengkapi alamat!"))); } }, style: ElevatedButton.styleFrom(backgroundColor: theme.primary, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: const Text("Simpan Alamat")))
              ]));
          });
      },
    );
  ...
  Widget _buildDropdown(AppThemeData theme, String? value, String hint, List<String> items, Function(String?) onChanged) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 16), decoration: BoxDecoration(color: theme.background, borderRadius: BorderRadius.circular(12)), child: DropdownButtonHideUnderline(child: DropdownButton<String>(value: value, hint: Text(hint, style: TextStyle(color: Colors.grey.shade400)), isExpanded: true, dropdownColor: theme.surface, items: items.map((e) => DropdownMenuItem(value: e, child: Text(e, style: TextStyle(color: theme.textMain)))).toList(), onChanged: onChanged)));
  }

  void _showMemberDetail(BuildContext context, AppThemeData theme) {
    bool isPro = _memberType == "Pro";
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (context) => Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: theme.background, borderRadius: const BorderRadius.vertical(top: Radius.circular(30))), child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [Text("Status Member", style: GoogleFonts.plusJakartaSans(fontSize: 20, fontWeight: FontWeight.bold, color: theme.textMain)), const Spacer(), Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(color: isPro ? const Color(0xFFFFD700).withOpacity(0.2) : Colors.grey.shade200, borderRadius: BorderRadius.circular(20), border: Border.all(color: isPro ? const Color(0xFFFFD700) : Colors.grey)), child: Text(isPro ? "PRO" : "REGULAR", style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.bold, color: isPro ? const Color(0xFFD4AF37) : Colors.grey)))]), const SizedBox(height: 20),
            Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: theme.surface, borderRadius: BorderRadius.circular(16), border: Border.all(color: theme.textMain.withOpacity(0.1))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Benefit Kamu Saat Ini:", style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.bold, color: theme.textMain)), const SizedBox(height: 10), _buildBenefitItem(Icons.check_circle, "Akses layanan dasar.", theme), _buildBenefitItem(Icons.check_circle, "Dukungan standar.", theme), if (!isPro) _buildBenefitItem(Icons.cancel, "Tidak ada gratis ongkir.", theme, isNegative: true), if (!isPro) _buildBenefitItem(Icons.cancel, "Antrian reguler.", theme, isNegative: true)])), const SizedBox(height: 24),
            if (!isPro) ...[
              Text("Upgrade ke PRO ✨", style: GoogleFonts.plusJakartaSans(fontSize: 18, fontWeight: FontWeight.bold, color: theme.textMain)), const SizedBox(height: 12),
              Container(width: double.infinity, padding: const EdgeInsets.fromLTRB(20, 20, 20, 0), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF1E1E2C), Color(0xFF2D2D44)]), borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: const Color(0xFF1E1E2C).withOpacity(0.3), blurRadius: 15, offset: const Offset(0, 8))]), child: Column(children: [Padding(padding: const EdgeInsets.only(bottom: 20), child: Row(children: [SizedBox(width: 60, height: 60, child: Lottie.asset('assets/lottie/trophy.json', errorBuilder: (c,e,s) => const Icon(Icons.emoji_events, color: Colors.amber, size: 40))), const SizedBox(width: 16), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Chupatu PRO", style: GoogleFonts.plusJakartaSans(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)), Text("Rp 49.000 / bulan", style: GoogleFonts.plusJakartaSans(color: const Color(0xFFFFD700), fontWeight: FontWeight.bold, fontSize: 14))]))])), const Divider(color: Colors.white24, height: 1), const SizedBox(height: 16), Padding(padding: const EdgeInsets.only(bottom: 20), child: Column(children: [_buildDarkBenefitItem("Gratis Antar Jemput"), _buildDarkBenefitItem("Prioritas Pengerjaan"), _buildDarkBenefitItem("Diskon 20%")])), ClipRRect(borderRadius: const BorderRadius.vertical(bottom: Radius.circular(24)), child: Material(color: const Color(0xFFFFD700), child: InkWell(onTap: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (context) => MemberPaymentPage(onPaymentSuccess: () => _saveFirestoreData('memberType', 'Pro')))); }, child: Container(height: 60, alignment: Alignment.center, child: Text("UPGRADE SEKARANG", style: GoogleFonts.plusJakartaSans(color: Colors.black, fontWeight: FontWeight.w900, fontSize: 16, letterSpacing: 1))))))])), const SizedBox(height: 40),
            ] else ...[
              Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.green)), child: Row(children: [const Icon(Icons.verified, color: Colors.green, size: 30), const SizedBox(width: 12), Expanded(child: Text("Status PRO Anda aktif.", style: GoogleFonts.plusJakartaSans(color: Colors.green.shade800, fontWeight: FontWeight.bold)))])), const SizedBox(height: 24),
              SizedBox(width: double.infinity, child: OutlinedButton(onPressed: () { Navigator.pop(context); showDialog(context: context, builder: (ctx) => AlertDialog(title: const Text("Batalkan Langganan?"), content: const Text("Anda akan kembali ke Regular."), actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text("Kembali")), ElevatedButton(onPressed: () { _saveFirestoreData('memberType', 'Regular'); Navigator.pop(ctx); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Paket dibatalkan."))); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.red, foregroundColor: Colors.white), child: const Text("Batalkan"))])); }, style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16), side: BorderSide(color: Colors.red.shade300), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: Text("Berhenti Langganan", style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.bold, color: Colors.red)))), const SizedBox(height: 40),
            ]
          ]))));
  }
