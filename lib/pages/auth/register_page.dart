...
class _RegisterPageState extends State<RegisterPage> {
  ...
  // --- 1. VALIDASI FORMAT EMAIL (REGEX) ---
  bool _isValidEmail(String email) {
    // Regex cek format standard
    final emailRegex = RegExp(r"^[a-zA-Z0-9.a-zA-Z0-9.!#$%&'*+-/=?^_`{|}~]+@[a-zA-Z0-9]+\.[a-zA-Z]+");
    return emailRegex.hasMatch(email);
  }

  // --- 2. VALIDASI NO HP (INDONESIA FORMAT) ---
  // Harus diawali '08' dan panjang total 10-14 angka
  bool _isValidPhone(String phone) {
    final phoneRegex = RegExp(r'^08[0-9]{8,12}$');
  ...
  Future<void> _signUp() async {
    ...
    // 2. Cek Validasi Email
    if (!_isValidEmail(email)) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Format email salah! (contoh: nama@gmail.com)'), backgroundColor: Colors.red));
      return;
    }

    // 3. Cek Validasi No HP (Format Indo)
    if (!_isValidPhone(phone)) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Nomor HP tidak valid! Harus diawali 08.. (Min 10 digit)'), backgroundColor: Colors.red));
      return;
    }

    // 4. Cek Terms
    if (!_agreedToTerms) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Anda harus menyetujui Syarat & Ketentuan'), backgroundColor: Colors.red));
      return;
    }
    ...
    try {
      // 5. Create User di Firebase Auth
      ...
      if (user != null) {
        // --- FITUR UTAMA: KIRIM VERIFIKASI EMAIL ---
        // Ini kuncinya! Kirim email ke alamat yang didaftarkan.
        ...
        await FirebaseFirestore.instance.collection('users').doc(user.uid).set({
          ...
          'photoURL': '',
          'isEmailVerified': false, // Penanda di database (opsional)
        });

        // 7. Logout Otomatis (PENTING!)
        // Supaya user tidak langsung masuk ke Home sebelum verifikasi.
        await FirebaseAuth.instance.signOut();

        if (mounted) {
          // Tampilkan Dialog Sukses & Instruksi
          showDialog(
            context: context,
                      ...
                      child: Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            Text('Create Account', style: GoogleFonts.plusJakartaSans(fontSize: 24, fontWeight: FontWeight.w800, color: textDark)),
                            const SizedBox(height: 4),
                            Text('Start your shoe care journey today.', style: GoogleFonts.plusJakartaSans(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.grey.shade500)),

                            const SizedBox(height: 24),

                            // INPUTS
                            _buildModernInput(controller: _nameController, label: 'Full Name', icon: Icons.person_outline_rounded),
                            const SizedBox(height: 16),
                            _buildModernInput(controller: _phoneController, label: 'WhatsApp Number', icon: Icons.call_outlined, inputType: TextInputType.phone),
                            const SizedBox(height: 16),
                            _buildModernInput(controller: _emailController, label: 'Email Address', icon: Icons.alternate_email_rounded, inputType: TextInputType.emailAddress),
                            const SizedBox(height: 16),
                                    ...
                                    TextSpan(
                                      ...
                                      children: [
                                        TextSpan(
                                          ...
                                          style: const TextStyle(fontWeight: FontWeight.bold, color: primaryBlue, decoration: TextDecoration.underline, decorationColor: primaryBlue),
                            ...
                            _isLoading
                              ...
                              decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), gradient: const LinearGradient(colors: [primaryBlue, accentCyan]), boxShadow: [BoxShadow(color: primaryBlue.withOpacity(0.4), blurRadius: 15, offset: const Offset(0, 8))]),
                              child: Material(
                                color: Colors.transparent,
                                child: InkWell(
                                  onTap: _signUp,
                                  borderRadius: BorderRadius.circular(20),
                                  child: Center(child: Text('Sign Up', style: GoogleFonts.plusJakartaSans(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white))),
                                ),
                              ),
                            ),
  ...
  Widget _buildModernInput({required TextEditingController controller, required String label, required IconData icon, bool isObscure = false, Widget? suffix, TextInputType inputType = TextInputType.text}) {
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: TextField(
            controller: controller, obscureText: isObscure, keyboardType: inputType,
